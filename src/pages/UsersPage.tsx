          ...
          <div className="overflow-x-auto w-full">
            ...
            <div className="md:hidden flex flex-col divide-y divide-slate-100">
               {isLoading ? (
                 ...
                 <div className="p-8 text-center text-slate-500 italic">No team members found matching your criteria.</div>
               ) : filteredUsers.map((user) => (
                 <div key={user.uid} className="p-4 flex flex-col gap-3 group hover:bg-slate-50/50 transition-colors">
                       ...
                       <div className="flex items-center gap-3">
                         ...
                         <div className="flex flex-col">
                           <span className="font-bold text-slate-900 leading-none">
                              {user.name} 
                              {user.uid === currentUserProfile?.uid && (
                                <Badge variant="outline" className="text-[10px] py-0 h-4 bg-blue-50 text-blue-600 border-blue-100 ml-1">You</Badge>
                              )}
                           </span>
                           <span className="text-xs text-slate-500 mt-1">@{(user as any).username || (user.email && user.email.split('@')[0]) || 'unknown'}</span>
                         </div>
                       </div>
                       
                        <DropdownMenu>
                          <DropdownMenuTrigger render={<Button variant="ghost" size="icon" className="h-8 w-8 -mr-2 -mt-1 text-slate-400 hover:text-slate-900 rounded-full" />}>
                              <MoreVertical className="w-4 h-4" />
                          </DropdownMenuTrigger>
                          <DropdownMenuContent align="end" className="w-56">
                            <DropdownMenuItem className="text-xs font-bold text-slate-400 uppercase tracking-widest pointer-events-none px-3 py-2">
                    ...
                    <div className="flex justify-between items-center text-xs mt-1 border-t border-slate-100 pt-2">
                       <span className="flex items-center gap-1.5 text-slate-600"><Building2 className="w-3.5 h-3.5" />{user.assignedProjects?.length || 0} Project(s)</span>
                       <span className="flex items-center gap-1 text-slate-500 font-mono tracking-tighter"><Calendar className="w-3.5 h-3.5" />{user.createdAt ? format(new Date(user.createdAt), 'MMM d, yyyy') : 'N/A'}</span>
                    </div>
                 </div>
               ))}
            </div>
